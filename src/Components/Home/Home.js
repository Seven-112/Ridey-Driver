...
import { View, Text, Image, TouchableOpacity, ActivityIndicator, Switch, Alert } from 'react-native';
...
const RenderRequest = ({ item, actions, setLoading, vehicle, userData, setAccepted }) => {
    ...
    return (
        <View style={styles.bottomRequestView} >
            ...
            <View style={styles.botButtonContainer}>
                ...
                <TouchableOpacity style={styles.acceptButton} onPress={() => handleAccepRequest(actions, userData, vehicle, setLoading, setAccepted, item)}>
                    <Text>Accept</Text>
                </TouchableOpacity>
...
const handelOffline = (actions, userData, setLoading, setOnline) => {
    const data = {
...
const handleOnline = (selectedVehicle, userData, actions, setOnline, online, setLoading) => {
    if (!selectedVehicle) {
...
const handleAccepRequest = (actions, driver, vehicle, setLoading, setAccepted, item) => {
...
const Home = ({ navigation, actions, userData, selectedVehicle, requests }) => {
    ...
    socket.on('pendingRequest',socketData=>{
        console.log("data from socket",socketData)
    })
    ...
    const [region, setRegion] = useState({ latitude: 31.481084, longitude: 74.311172 });
        ...
        <View style={styles.container}>
            ...
            <View style={styles.profileContainer}>
                ...
                <Switch
                    ...
                    onValueChange={
                        !online ?
                            () => handleOnline(selectedVehicle, userData, actions, setOnline, online, setLoading)
                            :
                            () => handelOffline(actions, userData, setLoading, setOnline)
                    }
                    value={online}
                />
            </View>
            ...
            {!accepted && online && requestData.length > 0 ?
                requestData.map((item, index) => <RenderRequest actions={actions} userData={userData} vehicle={selectedVehicle} setLoading={setLoading} setAccepte={setAccepted} index={index} item={item} />)
                :
                accepted && online && requestData.length > 0 ?
